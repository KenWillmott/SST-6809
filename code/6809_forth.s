...
;************************
; SST-6809 implementation map
;
; $0000-$00FF direct page (unused)
; $0100-$01FF scratchpad (unused)
; $0200 dictionary in RAM start (grows up)
; $A5C0 parameter stack start (grows down)
; $A5C0 TIB start (grows up)
; $A7C0 return stack start (grows down)
; $A7C0-$A7Fx user variables
; $A7Fx-$AFFF disk buffers
; $B000-$CFFF ROMable Forth
; $D000 ASSIST09 data area begins
...
; mass storage
NSCR    EQU   512-64         ; Number of 1024 byte screens ("blocks") on RAM disk
KBBUF   EQU   1024           ; Data bytes per disk sector (B/BUF)
NBUF    EQU   1              ; Number of buffers per screen (B/SCR)
CO      EQU   NBUF*KBBUF+4   ; Data bytes per buffer plus 4
BUFS	EQU   2              ; Number of buffers

RAMS    EQU   $0200          ; Start of RAM
ROMS	EQU   $B000          ; Start of ROM code
BUF1    EQU   ROMS-(CO*BUFS) ; First disk buffer (FIRST)
US      EQU   $0040          ; User variable space size
RTS     EQU   $0200          ; Return stack and terminal buffer size
INITR0  EQU   BUF1-US        ; Base of return stack
INITS0  EQU   INITR0-RTS     ; Base of parameter stack
EM      EQU   INITS0-$0400   ; End of dictionary + 1

; hardware equates

ACIAC   EQU   $EB00          ; ACIA Control
ACIAD   EQU   ACIAC+1        ; ACIA Data
MEMLAT	EQU   $EF00	; Paging latch
...
;*************************************************
        ORG   ROMS
ORIG    LDA   #$03           ; Reset the ACIA
        STA   ACIAC
        LDA   #$15           ; Configure the ACIA
        STA   ACIAC
        LDY   #ENTRY
        LDS   #INITR0         ; Initial stack pointers
        LDU   #INITS0
...
;        TFR   0,D	; illegal registers are '$FFFF' on a 6809, $0000 on a 6309
	FDB   $1FC0	; replace TFR instruction when assembler rejects illegal op code
        PSHU  D              ; Push it
        LDX   ,Y++           ; NEXT
        JMP   [,X++]
